...
RoundLatency RunOneRound(us3_turbo::client::Client& client, const Args& a,
  ...
  for (std::uint32_t i = 1; i <= num_parts; ++i) {
    ...
    ok = client.UploadPartGds(
        upload_id, i,
        ConstBufferView{.data = static_cast<char*>(data_buf) + off, .size = len},
        etag, error);
#else  // BENCH_UCX
...
void Worker(std::uint32_t wid, const Args& a,
  ...
  if (cudaError_t e = cudaMemcpy(dev, host_pattern.data(), a.total,
...
// ---- 打印与 CSV ----
void PrintSummary(const Args& a, std::uint32_t ok, std::uint32_t fail,
  ...
  std::cout << "=== results (" << kPathName << " multipart, conc="
            << a.concurrency << ", reps=" << a.reps << ") ===\n"
            << "  ok          : " << ok << "\n"
            << "  fail        : " << fail << "\n"
