...
struct GdsMpArgs : rtest::bench::BaseArgs {
  std::uint64_t part_size{rtest::kDefaultPartSize};
};

// ---- 单轮 multipart ----

RoundResult GdsMpRunOneRound(us3_turbo::client::Client& client, const GdsMpArgs& a,
                             std::uint32_t round_idx, std::uint32_t worker_idx, void* dev_buf) {
  using namespace us3_turbo::client;
  RoundResult lat;
  lat.bytes = a.total;

  const std::string key = a.key_prefix + "-gds-w" + std::to_string(worker_idx) +
  ...
  lat.setup_ms = ms_double(clk::now() - t_create0).count();
...
void GdsMpWorker(std::uint32_t wid, const GdsMpArgs& a, us3_turbo::client::Client& client,
  ...
  // GPU buffer + H2D
  void* dev = nullptr;
  if (cudaMalloc(&dev, a.total) != cudaSuccess) {
    std::cerr << "[w" << wid << "] cudaMalloc(" << rtest::HumanBytes(a.total) << ") failed\n";
    return;
  }
  if (cudaMemcpy(dev, host_pattern.data(), a.total, cudaMemcpyHostToDevice) != cudaSuccess) {
  ...
  for (std::uint32_t r = 0; r < a.warmup; ++r)
    (void)GdsMpRunOneRound(client, a, r, wid, dev);
  ...
  for (std::uint32_t r = 0; r < a.reps; ++r) {
    RoundResult lat = GdsMpRunOneRound(client, a, r, wid, dev);
...
bool GdsMpParseArgs(int argc, char** argv, GdsMpArgs& a) {
...
int main(int argc, char** argv) {
  ...
  GdsMpArgs a;
  if (!GdsMpParseArgs(argc, argv, a)) return 2;

  const std::uint32_t num_parts =
      static_cast<std::uint32_t>((a.total + a.part_size - 1) / a.part_size);

  if (!a.csv) {
    std::cout << "=== gds multipart bench ===\n"
              << "  proxy       : " << a.proxy << "\n"
  ...
  for (std::size_t w = 0; w < nworkers; ++w)
    threads.emplace_back(GdsMpWorker, static_cast<std::uint32_t>(w), std::ref(a), std::ref(client),
  ...
  if (a.csv) {
    rtest::bench::WriteCsv("gds", a, all, num_parts);
  } else {
    rtest::bench::PrintReport("gds multipart", all, wall_ms);
  }
