// rdma_put_bench.cpp — RDMA 单步 PUT 性能基准（rtest/bench/rdma）。
//
// 测量维度：
//   1. 吞吐 —— 多 worker 并发 RDMA PUT 聚合吞吐（MiB/s, ops/s）
...
//   RdmaMemoryManager 单例均线程安全）；每个 worker 线程拥有独立的 host
//   buffer，从共享原子计数器领取对象序号并发上传。
//   首次 PutObject 时在内部分配 RDMA QP + 注册 MR，无需显式 Register。
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--proxy") {
      ...
      std::cout << "usage: us3_turbo_bench_" << kPathName << "_put [options]\n"
                << "  --proxy HOST:PORT        proxy endpoint (default " << rtest::kDefaultProxy << ")\n"
...
int main(int argc, char** argv) {
  ...
  for (std::size_t w = 0; w < nworkers; ++w) {
    if (!stats[w].ready) {
      std::cerr << "[worker " << w << "] not ready (buffer alloc failed)\n";
      continue;
    }
    any_ready = true;
    ok += stats[w].ok;
