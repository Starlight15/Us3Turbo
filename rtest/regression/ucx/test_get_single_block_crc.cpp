...
int main(int argc, char** argv) {
  ...
  std::string put_etag;
  {
    ...
    if (!client.PutObject(put_req, ConstBufferView{.data = put_buf.data(), .size = size}, put_resp)) {
  ...
  // ---- GET ----
  {
    ...
    if (!client.GetObjectUcx(bucket, key, MutableBufferView{.data = get_buf.data(), .size = size}, get_res) ||
        !get_res.ok) {
