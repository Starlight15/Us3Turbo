...
class Client {
 public:
  ...
  [[nodiscard]] bool PutObject(const ClientProxyPutRequest& request, ConstBufferView buffer,
  ...
  [[nodiscard]] bool CreateMultipartUpload(const std::string& bucket, const std::string& key, PutDataPath path,
                                           std::string& out_upload_id, std::string& out_error) const;

  /** @brief GDS 路径上传单个 part：为本 part 独立注册 RDMA token。 */
  [[nodiscard]] bool UploadPartGds(const std::string& upload_id, std::uint32_t part_number, ConstBufferView buffer,
                                   std::string& out_etag, std::string& out_error) const;

  /** @brief UCX 路径上传单个 part：为本 part 独立注册 descriptor。 */
  [[nodiscard]] bool UploadPartUcx(const std::string& upload_id, std::uint32_t part_number, ConstBufferView buffer,
                                   std::string& out_etag, std::string& out_error) const;

  /** @brief 完成分段上传，返回最终 object_id/etag/size。 */
  [[nodiscard]] bool CompleteMultipartUpload(const std::string& upload_id, const std::vector<PartInfo>& parts,
                                             CompletedMultipart& out) const;

  /** @brief 终止分段上传，释放 proxy 会话（幂等）。 */
  [[nodiscard]] bool AbortMultipartUpload(const std::string& upload_id, std::string& out_error) const;

  // ===== GET 接口 =====

  /** @brief 查对象布局（GetObject 第一步），返回 object_size 供调用方分配buffer。 */
  [[nodiscard]] bool StatObject(const std::string& bucket, const std::string& key, std::uint64_t& out_object_size,
                                std::string& out_error) const;

  /**  @brief GDS 通路 GET：buffer 须已按 StatObject 返回的 size 分配。 */
  [[nodiscard]] bool GetObjectGds(const std::string& bucket, const std::string& key, MutableBufferView buffer,
                                  GetPathResult& result) const;

  /** @brief UCX 通路 GET：buffer 须已按 StatObject 返回的 size 分配（host内存）。 */
  [[nodiscard]] bool GetObjectUcx(const std::string& bucket, const std::string& key, MutableBufferView buffer,
                                  GetPathResult& result) const;
