 ...
 *
 * Mode B：client 只与 proxy 交互（OpenSession + GdsPut 都走 options.endpoint），
 * backend 对 client 透明。client_id / bearer_token / default_headers /
 * gds_data_endpoint 在 GDS-only 链路无服务端消费，已移除。只保留 GDS PUT
 * 必需的 endpoint / 超时 / 大小上限配置。
 */
struct ClientOptions {
  /** proxy 控制面 endpoint "host:port"（OpenSession + GdsPut 都走这里）。 */
  std::string endpoint;
