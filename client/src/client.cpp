...
[[nodiscard]] bool VerifyPartCrc32c(std::string_view request_id, ConstBufferView buffer,
  ...
  if (is_device) {
    ...
    if (cudaError_t e =
            cudaMemcpy(host.data(), buffer.data, buffer.size, cudaMemcpyDeviceToHost);
        e != cudaSuccess) {
    ...
    local = Crc32c(std::span<const std::byte>(static_cast<const std::byte*>(buffer.data),
                                              buffer.size));
  }
  if (local == remote_crc32c) {
    spdlog::info("{} (req={}): crc32c MATCH local={:08x} remote={:08x}", request_id, tag,
                 local, remote_crc32c);
    return true;
  }
  spdlog::error("{} (req={}): crc32c MISMATCH local={:08x} remote={:08x}", request_id,
...
bool Client::Initialize() {
  ...
  if (!proxy_->ok()) {
    spdlog::error("Initialize: proxy channel({}) init failed: {}", options_.endpoint,
                  proxy_->init_error());
    proxy_.reset();
    return false;
  }
...
bool Client::PutObject(const ClientProxyPutRequest& request, ConstBufferView buffer,
  ...
  if (ch == nullptr) {
    spdlog::error("PutObject: {} channel not initialized (req={})",
                  request.path == PutDataPath::kGds ? "GDS" : "UCX", request.request_id);
    return false;
  }
...
bool Client::CreateMultipartUpload(const std::string& bucket, const std::string& key,
  ...
  const ::us3_turbo::proxy::PutDataPath proto_path = (path == PutDataPath::kGds)
                                                         ? ::us3_turbo::proxy::PATH_GDS
                                                         : ::us3_turbo::proxy::PATH_UCX;
  return proxy_->CreateMultipartUpload(request_id, bucket, key, proto_path, out_upload_id,
                                       out_error);
}
...
bool Client::UploadPartGds(const std::string& upload_id, std::uint32_t part_number,
  ...
  if (options_.verify_crc32c && result.crc32c != 0) {
    VerifyPartCrc32c(request_id, buffer, result.crc32c, IsDevicePointer(buffer.data),
                     "UploadPartGds");
  }

  out_etag = result.etag;
  spdlog::info("UploadPartGds (req={}): upload={} part={} size={} etag={}", request_id,
               upload_id, part_number, buffer.size, out_etag);
  return true;
}
...
bool Client::UploadPartUcx(const std::string& upload_id, std::uint32_t part_number,
  ...
  PutPathResult result;
  const bool rpc_ok =
      proxy_->UploadPartUcx(request_id, upload_id, part_number, buffer.size,
                            desc.remote_addr, desc.rkey, desc.client_ucx_addr, result);
  if (!rpc_ok || !result.ok) {
  ...
  out_etag = result.etag;
  spdlog::info("UploadPartUcx (req={}): upload={} part={} size={} etag={}", request_id,
               upload_id, part_number, buffer.size, out_etag);
  return true;
}
