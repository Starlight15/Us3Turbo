 ...
 *
 * 调用侧每次（含每次重试）生成新 request_id 传入；bucket/key/timeout 直
 * 接取自 PutObjectRequest，不再经中间结构复制。
 *
...
class ProxyRpc : public RpcBase {
 public:
  ...
  [[nodiscard]] bool Put(const PutObjectRequest& request,
   ...
   */
  [[nodiscard]] bool RdmaPut(const PutObjectRequest& request,
                             std::string_view request_id,
                             std::chrono::milliseconds timeout,
