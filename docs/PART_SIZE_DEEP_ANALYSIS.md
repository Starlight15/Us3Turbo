...
## 0. 2026-08-07 复测校验(必读:RDMA 16M 结论已反转)

**触发**:本文档原写于 2026-08-06。2026-08-07 的 MR-pool 工作(commit 4fdd47a)顺带把 `PutFromRdma` 的 mock 路径改为**跳过 8M/16M memcpy**(`ac_server.cc:1797 if(!ioContext->mock_aio_write_) memcpy(...)`:mock 下 data_ 不落盘也无人读,这步纯冗余拷贝)。该 memcpy 原跑在 backend **主 EventLoop(串行)**,16M 时引发 ~16ms 主 loop backlog——正是原 §4.2/§6 归因的"16M 排队超线性"的物理根因。跳过后该罚项消失,**RDMA 16M 从"崩"反转为"最优"**。GDS 路径不受此改动影响(gds_service 另有 mock),GDS 结论不变。

**复测条件**(与原文一致:nt=8,cp=8,client 32,total 64M,reps 3 warmup 1,`mock_aio_write=1` 纯搬运;binary 含 4fdd47a):

| part | RDMA 吞吐(原→复测) | RDMA data-plane p50(原→复测) | RDMA per-part rpc_p50(原→复测) |
|---|---|---|---|
| 4M | 4225 → **3695**(-13%,会话方差上限) | 427 → 484ms | 28.13 → 32.13ms |
| 8M | 6148 → **6516**(+6%,噪声内) | 267 → 252ms | 35.71 → 32.09ms |
| 16M | 4056 → **7761**(**+91%**) | 391 → **190ms** | 97.80 → **48.28ms** |

**读数**:
- **RDMA 16M 单 part rpc 97.8→48.28ms(-51%),吞吐 +91%**:罚项消失。§3.1 成本模型里 RDMA 16M 的"超线性罚项 +46.9ms"已归零(48.28ms ≈ 线性预测 50.9ms)。曲线从"8M 峰值、16M 崩"变为**单调递增(16M 最优)**。
- 4M 略降 ~13%(3 次复测 3603/3709/3695 一致,非单次偶发;但机制未定位——4M memcpy 本就小,跳过收益微,疑会话方差或 proxy 侧漂移,不影响"4M 偏低=固定开销摊不薄"结论)。
- GDS 列(2525/2800/1425 vs 原 2444/3097/1381)在 ±10% 会话方差内,16M 仍崩(1425<2800),**GDS "8M 甜点"结论不变**。

**因此**:本文 §1/§3/§3.1/§4.2/§6 凡断言"RDMA 16M 崩 / 排队超线性 / 罚项 +46.9ms"处,均以本节为准——该现象是 mock 路径未跳 memcpy 时的历史结论,已被 4fdd47a 移除。**真写路径(`mock_aio_write=0`)16M 仍会落盘 + memcpy,结论可能不同,未测。**
...
> GDS 的 16M 崩得比 RDMA 狠(53 vs 164 MiB/s 单 part 净带宽,2026-08-06 原测),是因为 GDS 在(3)之外还额外撞上了 gds_service 的 4×16M buffer pool 与 cuobj token 路径争抢(见 §6)。
>
> **2026-08-07 复测(见 §0)已反转**:RDMA 16M 单 part 净带宽升至 331 MiB/s、吞吐 4056→7761,不再"崩"——原(3)的"排队超线性"是 mock 路径 16M memcpy 串行占用主 loop 的表象,4fdd47a 跳过该 memcpy 后罚项归零。GDS 16M 仍崩(路径未变)。
...
| RDMA | 4M | 2048 | 0.010ms | 0.526ms | 32.13ms | 52.95ms | 30.36ms | 124.5 MiB/s |
| RDMA | **8M** | 1024 | 0.010ms | 1.087ms | 32.09ms | 56.01ms | 31.62ms | **249.3 MiB/s** |
| RDMA | 16M | 512 | 0.011ms | 3.544ms | 48.28ms | 89.72ms | 50.80ms | 331.4 MiB/s |

> 上表 RDMA 三行为 2026-08-07 复测值(见 §0)。4M/8M 的 rpc_p50 几乎相等(~32ms),说明 8M 以内固定开销主导、size 项可忽略;16M 升至 48.28ms 但已无原 +46.9ms 超线性罚项(≈ §3.1 线性预测 50.9ms)。GDS 三行仍为 2026-08-06 原测(GDS 路径未改,复测在 ±10% 方差内,未重 trace)。
...
| RDMA(原 08-06) | ~20.5ms | 526 MB/s | 50.9ms | 97.8ms | **+46.9ms** |
| RDMA(复测 08-07) | ~32ms* | —* | 50.9ms(沿用旧拟合) | 48.28ms | **≈0**(48.28 ≤ 预测 50.9) |

> *复测 4M/8M 的 rpc_p50 几乎相等(32.13/32.09ms),固定开销 F 抬至 ~32ms 且 4M/8M 拟合退化为常数(BW 不可识别);16M 实际 48.28ms 已**低于**旧线性预测 50.9ms,即原 +46.9ms 超线性罚项已归零(见 §0:mock 路径跳 16M memcpy 移除了主 loop 串行)。
...
> ⚠️ 2026-08-07 复测(见 §0):RDMA 16M per-part rpc 已降至 48ms、吞吐 +91%,本节"RDMA 16M 98ms/排队超线性"为 mock 跳 memcpy 前的历史值。GDS 16M 仍崩(299ms 量级不变),本节对 GDS 的分析仍有效。
...
|---|---|---|---|---|
| 4M | 2444(原)/ 2525(复测) | 4225(原)/ **3695(复测)** | 756 / 734ms | 427 / 484ms |
| 8M | 3097(原)/ 2800(复测) | 6148(原)/ **6516(复测)** | 554 / 632ms | 267 / 252ms |
| 16M | 1381(原)/ 1425(复测) | 4056(原)/ **7761(复测)** | 1145 / 1163ms | 391 / 190ms |

(RDMA 列为 2026-08-07 复测,见 §0:16M 因 mock 路径跳 memcpy 而 +91%、结论反转。GDS 列复测在 ±10% 会话方差内、结论不变。trace 日志开销使吞吐比无 trace 时略低 1-2%。)
