...
| nt | cp | GDS(原→复测) | RDMA(原→复测) |
|---|---|---|---|
| 4 | 8 | 2953 → 3088 | 6229 → 6502 |
| 8 | 8 | 3097 → 2800 | 6148 → 6738 |
| 16 | 8 | 2802 → 3080 | 6369 → 6283 |
| 32 | 8 | 2582 → 2771 | 6320 → 6707 |
| 16 | 16 | 3564 → 3985 | 6533 → 6786 |

全部落在 ±12% 会话方差内(最大偏差 GDS nt16/cp16 +12%、GDS nt8 −10%,均为单次 3-rep 抖动;RDMA 全档 ±6%)。**所有结论不变**:nt>cp 时 GDS 反降(nt32<nt16)、nt=cp=16 为 GDS 最优、RDMA 对 nt 钝感。§3 原表保留为 08-06 历史值,不逐格替换(替换单一噪声样本无信息增益)。注意 8M/nt8/cp8 点 GDS 复测 −10% 系偶发偏低,非机制变化——同点 RDMA 反而 +6%。RDMA mock 路径跳 memcpy 的变化见 [PART_SIZE §0](PART_SIZE_DEEP_ANALYSIS.md#0-2026-08-07-复测校验必读rdma-16m-结论已反转),对 nt 扫描无实质影响(RDMA 本就钝感)。
