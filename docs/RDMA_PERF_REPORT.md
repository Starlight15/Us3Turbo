...
分段上传 part=4M / nt=8 / cp=32,真写 ≈ 3.56 GiB/s,真读 ≈ 2.5 GiB/s(4M 对象,conc=32)。**写 > 读**(与 GDS 相反:RDMA GET 路径单流串行,读带宽低于写)。真写被 NVMe 落盘拉低,搬运能力被掩盖(见 §2 mock)。
...
conc 16–64 扁平(2.3–2.5 GiB/s,±4%);**conc≥48 p95 飙至 119–132ms**。读带宽 < 写带宽(RDMA GET 路径单流串行,且 NVMe 读 + RDMA WRITE 两段)。
