...
**分段上传 part=4M / nt=8 / cp=32,真写稳态 ≈ 3.30 GiB/s(8 轮均值),真读稳态 ≈ 4.34 GiB/s(4M 对象/conc=32,6 轮均值)。读 > 写(NVMe 读快、无写放大)。真写最优 part∈{2M,4M}(两者噪声内等效,选 4M:part 数减半、对齐 `max_single_put_bytes=4M`)——与 mock(8M)相反。**
...
小 part 并行度高、≥8M 输给 NVMe 写延迟/排队,整体随 part 增大而降。**2M 与 4M 相差 ~6%(单轮噪声范围内)**:8 轮复测 part=4M 均值 ~3298(3242–3410)、part=2M 均值 ~3239(3152–3398),**两者实测等效**。mock 下 8M 为峰,因 mock 跳盘写、瓶颈是固定开销摊薄;**真写瓶颈是 NVMe,小 part 更优**。**选 part=4M**:与 2M 同效但 part 数减半(控制面开销↓)、对齐 client `max_single_put_bytes=4M`。
...
**part=4M / nt=8 / cp=32 → 8 轮均值 ~3298 MiB/s(3242–3410,±2.5%)**。nt/cp 扫描在 part=2M 下进行(§2.2/§2.3),因 2M/4M 实测等效,最优 nt=8/cp=32 直接适用于 part=4M;cp=48 仅 +0.1%,不值。
...
- **真写 part 峰在 {2M,4M} 而非 8M**:真写瓶颈=NVMe。小 part → 更多并行 AIO 更好地喂 NVMe 队列;≥8M 单 AIO 写延迟/tail 上升 + GDS worker(wt=4)串行更久 → 聚合反降。2M/4M 实测等效(8 轮均值 3239 vs 3298,噪声内),选 4M 以减半 part 数、对齐 `max_single_put_bytes=4M`。mock 跳盘写,瓶颈是固定开销摊薄 → 8M 才峰。**两 regime 甜点不同,mock 调参不可直接搬到真写。**
...
| 项 | mock 时代(旧) | **真写(本次推荐)** |
|---|---|---|
| `--multipart_part_size` | 8M | **4M** |
| `--num_threads` | 16 | **8** |
| `--backend_conn_pool_size` | 16 | **32** |
| `[gds] worker_threads` | 4 | 4(不变) |
| 稳态吞吐 | mock~4200(不可比) | **真写 ~3.30G / 真读 ~4.34G**(多轮均值) |
...
# 写:真写 multipart,part 须与 --multipart_part_size 一致(proxy 会拒不匹配的 part)
./build/rtest/bench/gds/us3_turbo_bench_gds_multipart --part-size 4M --total 64M --concurrency 32 --reps 3 --warmup 1
...
**写稳态(part=4M/nt=8/cp=32,8 轮)**:3288 / 3242 / 3255 / 3282 / 3410 / 3322 / 3277 / 3309 MiB/s → 均值 ~3298(区间 3242–3410,±2.5%,非常稳定)。另测 part=2M 8 轮均值 ~3239(3152–3398)——**4M 与 2M 实测等效**,验证 §2.1 选 4M 的判断。§2.4 原 3736 为单轮最优值,多轮稳态偏低 ~13%。

**读稳态(4M 对象/conc=32,6 轮)**:4310 / 4372 / 4553 / 4597 / 4014 / 4212 MiB/s → 均值 ~4343(区间 4014–4597,±7%)。§3 报 5318 → 偏低约 18%,**5318 峰值 6 轮内不可复现**(实测最高 4597)。读波动显著大于写,conc=32 单次易受 client MR re-register 抖动影响。
...
**结论**:原文档趋势性结论(真写 part 小为优、2M/4M 等效、读>写、conc↑→p95↑)经多轮复测成立;分段上传已定为 part=4M,稳态写 ~3.30G / 读 ~4.34G(多轮均值),引用单轮峰值(3736/5318)应标注"单次最优"。盘累计已写 81.25 TB,处纯 TLC 稳态,与原文前提一致,故偏低非盘态从空变满所致。
