...
#### 1.2.2 num_threads(part=4M,cp=32 固定)
...
nt 4–32 全程扁平(±1.5%,nt=32 略高但差异 <1%)。真写瓶颈在 NVMe 不在 proxy 调度。**选 nt=8**(省线程,够用有余量)。

#### 1.2.3 backend_conn_pool_size(part=4M,nt=8 固定)
...
cp 4–48 扁平(3213–3374,±2.5%):cp=4 略低、cp≥8 均在 ~3300。4M 下 conn 占用时间对吞吐影响小于 2M(2M 时 cp 单调升、32–48 饱和)。**选 cp=32**(给足连接余量,差异 <3%)。
