#!/usr/bin/env bash
# test_ucx_put.sh — 验证 UCX PUT 链路端到端。
#
# 链路：client(host 内存) → proxy → backend(ucp_get_nbx 反向拉)。
# 与 GDS 链路独立。backend/proxy 二进制同时承载 gds+ucx 两条链路
...
BACKEND_PORT="9200"
RDMA_PORT="18616"   # cuObjServer GDS 端口(ucx 链路不用它,但 backend 启动需要)
...
for pid_var in BACKEND_PID PROXY_PID; do
  ...
  if ! kill -0 "$pid" 2>/dev/null; then
    ...
    echo "=== backend_ucx_test.log ==="; cat "${LOG_DIR}/backend_ucx_test.log"
    echo "=== proxy_ucx_test.log ===";  cat "${LOG_DIR}/proxy_ucx_test.log"
    exit 1
  fi
done
...
echo ""
if [[ $RC -eq 0 ]]; then
  echo "✅ [SUCCESS] UCX PUT test passed"
else
  echo "❌ [FAILED] UCX PUT test failed (exit=$RC)"
fi

echo ""
echo "=== backend_ucx_test.log (last 30 lines) ==="
tail -30 "${LOG_DIR}/backend_ucx_test.log"
echo ""
echo "=== proxy_ucx_test.log (last 30 lines) ==="
tail -30 "${LOG_DIR}/proxy_ucx_test.log"
