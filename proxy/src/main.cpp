...
void InitLogging() {
  ...
  us3_turbo::proxy::Logger::Init(level, static_cast<std::size_t>(FLAGS_log_max_size_mb),
...
std::unique_ptr<us3_turbo::proxy::AssembledStack> AssembleServices() {
  ...
  auto single_put = std::make_unique<us3_turbo::proxy::SinglePut>(stack->index.get(),
                                                                  stack->ufile_ac.get());
  auto multipart = std::make_unique<us3_turbo::proxy::Multipart>(stack->index.get(),
                                                                 stack->ufile_ac.get());
  auto get_object = std::make_unique<us3_turbo::proxy::GetObject>(stack->index.get(),
                                                                  stack->ufile_ac.get());
...
bool StartServer(brpc::Server& server, us3_turbo::proxy::ProxyService& service) {
  ...
  const std::string endpoint = FLAGS_bind_host + ":" + std::to_string(FLAGS_proxy_port);
