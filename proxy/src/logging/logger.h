...
// 按 [函数名] / [函数名][req=request_id] 前缀全链路追踪；LOG_* 宏自动填 __func__。
//
// 与 access_logger 分离：App 日志人类可读、按大小滚动、受级别控制；
// Access 日志结构化、按天切分、永久开启（见 access_logger.h）。
//
// 两类宏：
// - LOG_INFO/WARN/ERROR/DEBUG(rid, fmt, ...)  请求日志，输出 [func][req=rid] 消息
// - LOG_SYS_INFO/WARN/ERROR/DEBUG(fmt, ...)   进程日志（main 启动/停止、构造期等
...
class Logger {
 public:
  ...
  template <typename... Args>
  static void SysWarn(std::string_view func,
                      fmt::format_string<Args...> fmt, Args&&... args) {
    spdlog::warn("[{}] {}", func,
                 fmt::format(fmt, std::forward<Args>(args)...));
  }
  ...
  template <typename... Args>
  static void SysError(std::string_view func,
                       fmt::format_string<Args...> fmt, Args&&... args) {
    spdlog::error("[{}] {}", func,
                  fmt::format(fmt, std::forward<Args>(args)...));
  }
  ...
  template <typename... Args>
  static void SysDebug(std::string_view func,
                       fmt::format_string<Args...> fmt, Args&&... args) {
    spdlog::debug("[{}] {}", func,
                  fmt::format(fmt, std::forward<Args>(args)...));
  }
