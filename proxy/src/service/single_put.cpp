...
int SinglePut::ValidateGdsRequest(const ClientProxyPutRequest& req) {
  ...
  if (req.object_size() == 0 ||
      req.object_size() >
          static_cast<std::uint64_t>(FLAGS_max_single_put_bytes)) {
    LOG_WARN(rid, "object_size={} out of range [1, {}] bucket={}/{}",
             req.object_size(), FLAGS_max_single_put_bytes, req.bucket(),
             req.key());
    return PROXY_ERR_INVALID_PARAM;
  }
...
int SinglePut::ValidateUcxRequest(const ClientProxyPutRequest& req) {
  ...
  if (usrc.remote_addr() == 0 || usrc.packed_rkey().empty() ||
    ...
    LOG_WARN(rid, "ucx source fields incomplete bucket={}/{}", req.bucket(),
             req.key());
    return PROXY_ERR_MISSING_SOURCE;
  }
  return 0;
...
int SinglePut::PutGds(const ClientProxyPutRequest& req, PutOutput& out) {
  ...
  const auto res = client_->PutBlockGds(
      block_key, req.gds_source().rdma_token(), 0, req.object_size());
  if (res.ret_code != 0) {
...
int SinglePut::PutUcx(const ClientProxyPutRequest& req, PutOutput& out) {
  ...
  const auto& usrc = req.ucx_source();
  const auto res =
      client_->PutBlockUcx(block_key, usrc.remote_addr(), usrc.packed_rkey(),
...
bool SinglePut::WriteObjectIndex(const std::string& request_id,
  ...
  /* 写 fileidx_col */
  bool success =
      index_->InsertFileIdx(bucket, key, obj_id,
                            object_size,  // block_size = 对象大小（单块）
                            object_size, hash);

  if (!success) {
    LOG_ERROR(request_id,
              "Failed to write fileidx for key={}, rolling back block={}", key,
              obj_id + "_0");
    ...
    if (result.ret_code != 0) {
      LOG_WARN(request_id,
               "DeleteBlock failed during rollback: key={} error={}", block_key,
               result.error);
    }
    out.etag.clear();  // 标记失败
    return false;
