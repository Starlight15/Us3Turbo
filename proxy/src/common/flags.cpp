...
DEFINE_int32(backend_timeout_ms, 30000, "Timeout (ms) for proxy→backend forward (GdsPut/UcxPut/PutBlock)");
...
DEFINE_int64(upload_ttl_scan_interval_ms, 3600 * 1000, "interval (ms) between multipart TTL cleanup scans");
...
DEFINE_int32(backend_conn_pool_size, 8,
             ...
             "throughput)");
DEFINE_string(dbgate_endpoint, "192.168.1.100:20165", "DBGate proxy endpoint for MongoDB operations");
DEFINE_int32(dbgate_timeout_ms, 5000, "DBGate request timeout in milliseconds");
DEFINE_int32(dbgate_conn_pool_size, 4, "DBGate connection pool size (recommend: match num_threads)");
