...
// 每个 RPC 入口生成 snowflake trace_id(proxy 生成,对齐 s3proxy TraceId),
// 写入 CurrentTraceId()(同线程叶层 dbgate/ufile-ac/minit 取用),并在响应里
// 回 client。rid 为 trace_id 的十进制串,供 LOG/AccessLogger 关联。
#define TRACE_INIT()                                       \
  const std::uint64_t trace_id = GenTraceId();             \
  CurrentTraceId() = trace_id;                             \
  const std::string rid = std::to_string(trace_id);       \
...
void ProxyService::UploadPartGds(google::protobuf::RpcController* cntl_base,
  ...
  int ret =
      multipart_->UploadPartGds(rid, request->upload_id(), request->part_number(),
  ...
  response->set_bytes_written(out.bytes);
  if (out.crc32c != 0) response->set_crc32c(out.crc32c);
  LOG_INFO(rid, "success part={} etag={} bytes={}", request->part_number(), out.etag, out.bytes);
...
void ProxyService::UploadPartRdma(google::protobuf::RpcController* cntl_base,
  ...
  UploadPartOutput out;
  int ret = multipart_->UploadPartRdma(rid, request->upload_id(), request->part_number(),
                                       request->part_size(), request->rdma_token(), out);
  const auto latency = utils::ElapsedMs(start);
